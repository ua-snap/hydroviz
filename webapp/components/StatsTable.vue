...
<template>
    ...
    <div class="parameter">
      <div class="select mb-5 mr-3">
        <select id="scenario" v-model="scenarioInput">
          <option v-for="scenario in Object.keys(scenarios)" :value="scenario">
            {{ scenarios[scenario] }}
          </option>
        </select>
      </div>
  ...
  >
    <table class="table mb-6">
      ...
      <tr v-for="stat in streamflowStatistics">
        <th class="p-5" scope="row">
          <code>{{ stat.id }}</code>
        </th>
        <td class="p-5" v-html="stat.description"></td>
        <td class="p-5">
          {{
            Number(
              streamStats.data[lcInput][modelInput]['historical']['1976-2005'][
                stat.id
              ]
            )
          }}
        ...
        <td v-for="era in Object.keys(eras)" class="p-5">
          {{
            Number(
              streamStats.data[lcInput][modelInput][scenarioInput][era][stat.id]
            )
          }}
          <span style="color: #888" v-html="stat.units_short"></span>
        </td>
